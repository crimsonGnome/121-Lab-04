...
int main() {
  ...
  for (int i = 0; i < num_bubbles; i++) {
    ...
    std::cout << "What y? ";
    std::cin >> y;
    // use map string color as key to get Color class
    graphics::Color rgbColor = colors[color];
    // Bubble Constructor
    Bubble bubble(rgbColor, size, x, y);
    // push to vector
  ...
  for (int i = 0; i < num_bubbles; i++) {
    ...
    image.DrawCircle(bubbles[i].GetX(), bubbles[i].GetY(), bubbles[i].GetSize(),
                     bubbles[i].GetColor());

    // 2. Print the Bubble to the terminal using the ToString() method.
    //
    std::cout << bubbles[i].ToString();
  }
